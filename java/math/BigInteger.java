...
public class BigInteger extends Number implements Comparable<BigInteger> {
	 ...
	 *  1 : 正
	 * 
	 * Note that the BigInteger zero <i>must</i> have a signum of 0.
	 *
	 * BigInter の zero は signum が 0 になります。
	 * 
	 ...
	 *
	 * エンディアン http://ja.wikipedia.org/wiki/%E3%82%A8%E3%83%B3%E3%83%87%E3%82%A3%E3%82%A2%E3%83%B3
	 *
	 *   エンディアン (endianness) とは、
	 *   多バイトのデータ（即ち基本情報記憶単位を超えるデータ）を
	 *   メモリ上に配置する方式の種類のこと。
	 *
	 ...
	 *
	 *   ビッグエンディアンは人間にとって直観的にわかりやすいという利点があり、
	...
	BigInteger(char[] val) {
		...
		// 期待される配列のサイズを事前に割り出す
		int numWords;
		// 引数で与えられた char の長さが 10 より小さい場合は 1
		if (len < 10) {
			numWords = 1;
		} 
		// それ以外はある程度の要領を算出する
		//
		// x >>> a
		//   x のビットを a ビットだけ右にシフトさせる。左端には0が入る。
		//   左端に 0 が入るので符号の区別がない演算に使用します。(論理右シフト)
		//   http://www.hellohiro.com/basic/shift.htm
		else {    
